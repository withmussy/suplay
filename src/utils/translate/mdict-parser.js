...
import { REGEXP_STRIPKEY, getExtension , log} from 'utils/translate/mdict-common';
...
export default function MParser () {
  ...
  function parse_mdict(file, ext) {
      ...
      var methods = {
        ...
        readBlock: function(len, expectedBufSize, decryptor) {
          ...
          if (comp_type === 0) {
            ...
            tmp = comp_type === 2 ? inflate(tmp) : decompress(tmp, expectedBufSize, 1308672);
            this.forward(len);
    ...
    function read_header_sect(input, len) {
      ...
      log('dictionary attributes: ', attrs);
      config();
...
console.log('trail: ', _trail);          
    ...
    }).then(function(keyword_summary) {           log(keyword_summary);
    ...
    }).spread(function (record_summary) {       log(record_summary);
    ...
    }).spread(function() {                      log('-- parse done --', file.name);
